"""Create adjustment transaction command."""
...
class CreateAdjustmentCommand:
    ...
    def execute(self, data: CreateAdjustmentInput) -> Transaction:
        account = self._accounts.get_by_id(data.organization_id, data.account_id)
        if account is None:
            raise AccountNotFoundError("Account not found.")
        transaction = Transaction.create_adjustment(
